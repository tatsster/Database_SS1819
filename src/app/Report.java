...
public class Report {
	...
	public String toString(String inputPID) throws SQLException {
		try {
			rs = qr.executeQuery("SELECT * FROM  ACCOUNT1.PATIENT, (SELECT * FROM ACCOUNT1.OUTPATIENT WHERE PID_OUT ="
					+ inputPID + ") O WHERE PID = O.PID_OUT");
		} catch (SQLException e) {
		...
		if (rs.next() == true) {
			...
			try {
				do {
					String EID = rs.getString("EID_DOC");// if data is null `output` would be null, so there is no
															// chance of NPE unless `rs` is `null`
					...
					System.out.format(leftAlignFormat, " Patient's Address: ",
			...
			try {
				rs = qr.executeQuery(
						"SELECT EXID , EXDATE, EXDiAGNOSIS, MNAME, MEFFECTS, EXSECONDEXAMINATIONDATE, EXFEE, MPRICE FROM (SELECT * FROM  ACCOUNT1.EXAMINATION E, "
			...
			System.out.format(
					"|                                                                                                                                                                                                                                                               |%n");
			System.out.format(
					"| ------------------------------------------------------------------------------------------------------------------  PAYMENT REPORT OF THE PATIENT ------------------------------------------------------------------------------------------------------------|%n");
			System.out.format(
					"|                                                                                                                                                                                                                                                               |%n");
			System.out.format(
					"%n");
			System.out.format(leftAlignFormat_String, "Examination ID", "Examination Date", "Examination Diagnosis",
					"Medication Name", "Medication Effects", "Second Examination Date", "Examination Fee",
					"Medication Price", "Total");
			Long total = (long) 0;

			try {
				while (rs.next()) {
					Long EXFEE = rs.getLong("EXFEE");// if data is null `output` would be null, so there is no chance of
														// NPE unless `rs` is `null`
					Long MPRICE = rs.getLong("MPRICE");// if data is null `output` would be null, so there is no chance
														// of NPE unless `rs` is `null`
					total = total + EXFEE + MPRICE;

					System.out.format(leftAlignFormat, rs.getString("EXID"), rs.getString("EXDATE"),
							rs.getString("EXDIAGNOSIS"), rs.getString("MNAME"), rs.getString("MEFFECTS"),
							rs.getString("EXSECONDEXAMINATIONDATE"), rs.getLong("EXFEE"), rs.getLong("MPRICE"),
							(EXFEE + MPRICE));
			...
			try {
				rs = qr.executeQuery("SELECT * FROM  ACCOUNT1.PATIENT, (SELECT * FROM ACCOUNT1.INPATIENT WHERE PID_IN ="
						+ inputPID + ") I WHERE PID = I.PID_IN");
			} catch (SQLException e) {
				System.out.println("Query Failed! Check your connection or the valid of your query statement");
				e.printStackTrace();
				return "NULL";
			...
			if (rs.next() == true) {
				...
				try {
					do {
						...
						String NUR = rs.getString("EID_NUR");// if data is null `output` would be null, so there is no
																// chance of NPE unless `rs` is `null`
				...
				try {
					rs = qr.executeQuery(
							"SELECT TRID , TRSTART, TREND, TRRESULT, PADMISSIONDATE, PDISCHARGEDATE, PDIAGNOSIS, PSICKROOM, MNAME, MEFFECTS, PFEE, MPRICE "
									...
									+ "T.PID_IN = A.USES_PID_IN AND T.TRID = A.USES_TRID) F WHERE F.PID_IN = "
									+ inputPID);
				...
				System.out.format(
						"%n");
				System.out.format(leftAlignFormat_String, "Treatment ID", "Treatment Start", "Treatment End",
						"Treatment Result", "Admission Date", "Discharge Date", "Treatment Diagnosis",
						"Treatment Sickroom", "Medication Name", "Medication Effects", "Treatment Fee",
						"Medication Price", "Total");
				Long total = (long) 0;

				try {
					while (rs.next()) {
						Long PFEE = rs.getLong("PFEE");// if data is null `output` would be null, so there is no chance
														// of NPE unless `rs` is `null`
						Long MPRICE = rs.getLong("MPRICE");// if data is null `output` would be null, so there is no
															// chance of NPE unless `rs` is `null`
						total = total + PFEE + MPRICE;

						System.out.format(leftAlignFormat, rs.getString("TRID"), rs.getString("TRSTART"),
								rs.getString("TREND"), rs.getString("TRRESULT"), rs.getString("PADMISSIONDATE"),
								rs.getString("PDISCHARGEDATE"), rs.getString("PDIAGNOSIS"), rs.getString("PSICKROOM"),
								rs.getString("MNAME"), rs.getString("MEFFECTS"), rs.getLong("PFEE"),
								rs.getLong("MPRICE"), (PFEE + MPRICE));

					}

					System.out.format(
							"%n");
					System.out.format(leftAlignFormat_Final, " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ", " ",
							total);
					System.out.format(
							"%n");
				} catch (SQLException e) {
